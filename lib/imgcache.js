...
(function ($) {
  ...
  Private.setNewImgPath = function ($img, new_src, old_src) {
    ...
    Private.setNewAPath = function ($a, new_href, old_src) {
        DomHelpers.setAttribute($a, 'href', new_href);
        // store previous url in case we need to reload it
        DomHelpers.setAttribute($a, OLD_SRC_ATTR, old_src);
    };
  ...
  Private.getBackgroundImageURL = function ($div) {
    ...
    var regexp = /url\s?\((.+)\)/;
    var regexResults = regexp.exec(backgroundImageProperty);
    if(!regexResults || regexResults.length < 2){
      return null;
    }

    var img_src = regexResults[1];
  ...
  ImgCache.isCordova = function(){

    return Helpers.isCordova();
  };

  ImgCache.cacheFileUpload = function(file, url, success_callback, error_callback){

      var localPath = Private.getCachedFilePath(Helpers.sanitizeURI(url));
      var fileTransfer = new Private.FileTransferWrapper(ImgCache.attributes.filesystem);

      fileTransfer.filesystem.root.getFile(localPath, { create:true }, function (fileEntry) {
          fileEntry.createWriter(function (writer) {
              writer.onerror = error_callback;
              writer.onwriteend = function () { success_callback(fileEntry);  };
              writer.write(file, error_callback);
          }, error_callback);
      }, error_callback);
  };

  ImgCache.uploadCachedFile = function(file, url, options, success_callback, error_callback){

      var transfer = new Private.FileTransferWrapper(ImgCache.attributes.filesystem);
      if(!transfer.fileTransfer){
        error_callback('No fileTransfer available');
        return;
      }

      var localPath = Private.getCachedFileFullPath(Helpers.sanitizeURI(file));

      var uploadOptions = new FileUploadOptions();
      uploadOptions.params = options.params;
      uploadOptions.headers = options.headers;

      transfer.fileTransfer.upload(localPath, url, success_callback, error_callback, options);
  };
  ...
  ImgCache.useCachedFile = function ($img, success_callback, error_callback) {
    ...
    ImgCache.useCachedFileHref = function ($a, success_callback, error_callback) {
        if (!Private.isImgCacheLoaded()) {
            return;
        }

        Private.loadCachedFile($a, DomHelpers.getAttribute($a, 'href'), Private.setNewAPath, success_callback, error_callback);
    };
